...
#SBATCH --job-name=fmriprep_study-retinotopy_sub-01_ses-003.job
#SBATCH --output=/lustre03/project/6003287/datasets/cneuromod_processed/fmriprep/retinotopy/code/fmriprep_study-retinotopy_sub-01_ses-003.out
#SBATCH --error=/lustre03/project/6003287/datasets/cneuromod_processed/fmriprep/retinotopy/code/fmriprep_study-retinotopy_sub-01_ses-003.err
#SBATCH --time=4:00:00
...
datalad containers-run -m 'fMRIPrep_sub-01/ses-003' -n containers/bids-fmriprep --input sourcedata/retinotopy/sub-01/ses-003/fmap/ --input sourcedata/retinotopy/sub-01/ses-003/func/ --input sourcedata/templateflow/tpl-MNI152NLin2009cAsym/ --input sourcedata/templateflow/tpl-OASIS30ANTs/ --input sourcedata/templateflow/tpl-fsLR/ --input sourcedata/templateflow/tpl-fsaverage/ --input sourcedata/templateflow/tpl-MNI152NLin6Asym/ --output . --input 'sourcedata/smriprep/sub-01/anat/' --input sourcedata/smriprep/sourcedata/freesurfer/fsaverage/ --input sourcedata/smriprep/sourcedata/freesurfer/sub-01/ -- -w ./workdir --participant-label 01 --anat-derivatives ./sourcedata/smriprep --fs-subjects-dir ./sourcedata/smriprep/sourcedata/freesurfer --bids-filter-file code/fmriprep_study-retinotopy_sub-01_ses-003_bids_filters.json --output-layout bids  --use-syn-sdc --output-spaces MNI152NLin2009cAsym T1w:res-iso2mm --cifti-output 91k --notrack --write-graph --skip_bids_validation --omp-nthreads 8 --nprocs 8 --mem_mb 32768 --fs-license-file code/freesurfer.license --resource-monitor sourcedata/retinotopy ./ participant 
fmriprep_exitcode=$?
...
if [ -e $LOCAL_DATASET/resource_monitor.json ] ; then cp $LOCAL_DATASET/resource_monitor.json /scratch/bpinsard/fmriprep_study-retinotopy_sub-01_ses-003_resource_monitor.json ; fi 
if [ $fmriprep_exitcode -ne 0 ] ; then cp -R $LOCAL_DATASET /scratch/bpinsard/fmriprep_study-retinotopy_sub-01_ses-003 ; fi 
