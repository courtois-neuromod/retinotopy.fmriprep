...
#SBATCH --job-name=fmriprep_study-retinotopy_sub-05.job
#SBATCH --output=/lustre04/scratch/bpinsard/cneuromod.retinotopy.fmriprep/code/fmriprep_study-retinotopy_sub-05.out
#SBATCH --error=/lustre04/scratch/bpinsard/cneuromod.retinotopy.fmriprep/code/fmriprep_study-retinotopy_sub-05.err
...
datalad containers-run -m 'fMRIPrep_sub-05' -n bids-fmriprep --input sourcedata/templateflow/tpl-MNI152NLin2009cAsym/ --input sourcedata/templateflow/tpl-OASIS30ANTs/ --input sourcedata/templateflow/tpl-fsLR/ --input sourcedata/templateflow/tpl-fsaverage/ --input sourcedata/templateflow/tpl-MNI152NLin6Asym/ --output . --input 'sourcedata/retinotopy/sub-05/fmap/' --input 'sourcedata/retinotopy/sub-05/func/'  --input 'sourcedata/smriprep/sub-05/anat/' --input sourcedata/smriprep/sourcedata/freesurfer/fsaverage/ --input sourcedata/smriprep/sourcedata/freesurfer/sub-05/ -- -w ./workdir --participant-label 05 --anat-derivatives sourcedata/smriprep --fs-subjects-dir sourcedata/smriprep/sourcedata/freesurfer --bids-filter-file code/fmriprep_study-retinotopy_sub-05_bids_filters.json --output-layout bids  --use-syn-sdc --output-spaces MNI152NLin2009cAsym T1w:res-iso2mm --cifti-output 91k --notrack --write-graph --skip_bids_validation --omp-nthreads 8 --nprocs 12 --mem_mb 45056 --fs-license-file code/freesurfer.license  sourcedata/retinotopy ./ participant 
fmriprep_exitcode=$?
...
if [ -e $LOCAL_DATASET/workdir/fmriprep_wf/resource_monitor.json ] ; then cp $LOCAL_DATASET/workdir/fmriprep_wf/resource_monitor.json /scratch/bpinsard/fmriprep_study-retinotopy_sub-05_resource_monitor.json ; fi 
if [ $fmriprep_exitcode -ne 0 ] ; then cp -R $LOCAL_DATASET /scratch/bpinsard/fmriprep_study-retinotopy_sub-05 ; fi 
